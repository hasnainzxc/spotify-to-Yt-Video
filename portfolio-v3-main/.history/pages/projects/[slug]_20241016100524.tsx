...
export default function Project({
  ...
  const renderScreenShotList = useCallback(
    (screenshot: string) => {
      ...
      // Check if the screenshot is a local image or an external (Imgur) image
      const isImgurImage =
        screenshot.startsWith('https://') ||
        screenshot.startsWith('i.imgur.com');
      const imageUrl = isImgurImage
        ? screenshot
        : `${LOCAL_IMAGE_BASE_URL}${screenshot}`;

      // Check if the image is a video
      const isVideo = imageUrl.endsWith('.mp4'); // Adjust this condition based on your needs
  ...
  const IMGUR_BASE_URL = 'https://i.imgur.com/';
  const LOCAL_IMAGE_BASE_URL = '/static/projects/';
