        ...
        while retry_count < max_retries:
            try:
                ...
                if retry_count == max_retries:
                    # Check for 404/private playlist
                    if hasattr(e, 'http_status') and getattr(e, 'http_status', None) == 404:
                        print("\n[ERROR] Spotify playlist not found or is private. Please make sure the playlist is public or you have access to it.")
                    else:
                        print(f"\n[ERROR] Failed to fetch playlist after {max_retries} attempts: {e}")
...
def main():
    ...
    while True:
            ...
            if resume == 'y':
                if playlist_id:
                    ...
                    while True:
                        ...
                        if playlist_title:
                            break
                        print("Playlist title cannot be empty. Please enter a valid title.")
        ...
        again = input("\nWould you like to convert another Spotify playlist? (y/n): ").strip().lower()
        if again != 'y':
            print("Goodbye!")
            break
